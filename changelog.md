...
公式ドキュメント4種（best-practices/sub-agents/hooks/memory）を取得。外部GitHubソースはMCPスコープ制限により今週も取得不可。バージョンは **v2.1.222+** から進行が確認できなかった（CHANGELOG 取得不可）。今週の最大の発見は、**サブエージェントのデフォルトネスト深さが 3 層**（メインセッション下3段まで）であることが公式ドキュメントで明記されたこと。我々の知識ベースに「最大5段階（v2.1.172+）」と誤記していた内容を訂正した。第2の新規明文化は**同時実行サブエージェントのデフォルト上限が 20**であること（`CLAUDE_CODE_MAX_CONCURRENT_SUBAGENTS` で変更可能、`ultracode` 有効時は免除）。また `/subtask` コマンドがフォーク型サブエージェント起動として公式確認され、`Agent(fork)` deny ルールでフォークのみを禁止できることも明記された。バックグラウンドサブエージェントの利用可能ツールセット（MCP ツール＋19種 built-in）も列挙された。今週は新たな機能追加よりも既存ドキュメントの明確化・誤記訂正が主な成果となった。
