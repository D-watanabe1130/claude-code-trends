...
- **`/import` コマンド（v2.1.213+）**: Cursor/Copilot/AGENTS.md/Devin/Windsurf/Cline の設定を一括移行。指示ファイル・MCPサーバー・コマンド・サブエージェント・スキルが対象。
- **CLAUDE.md ブロックレベル HTML コメント除去**: `<!-- ... -->` はコンテキスト注入前に削除される（コードブロック内は保持）。保守者向けメモをコンテキストコストなしに記録可能に。
- **v2.1.222+**: `availableModels` allowlist でブロックされたサブエージェントのモデルファミリーエイリアスを、allowlist が許可する最新バージョンへ適切に代替するよう改善。
- **`CLAUDE_CODE_NEW_INIT=1` の詳細仕様確認**: 他エージェントルール（AGENTS.md/Devin/Windsurf/Cline）も読み込み、承認フローを経てファイル書き込み。
